...
\AtBeginDocument{%
  \hypersetup{%
    colorlinks=true,
    allcolors=blue,
    pdftitle={Coherent Projector-Overlap QFNNs with Chebyshev Responses: Expressivity, Approximation, and Coherent Depth},
    pdfauthor={Andrej Sum-Shik}
  }%
}
...
Expressivity, Approximation, and Coherent Depth}

\author*[1]{\fnm{Andrej} \sur{Sum-Shik}}
\email{asumshik@gmail.com}

\affil[1]{\orgdiv{MSc Programme in Quantum Information Science,
  Faculty of Science}, \orgname{University of Copenhagen},
  \orgaddress{\city{Copenhagen}, \country{Denmark}}}
...
\keywords{Quantum machine learning, quantum neural networks, Chebyshev
polynomials, alternating reflections, universal approximation, coherent
quantum circuits}
...
For fixed preceding parameters $\theta_{<\ell}$, let
$U_{\theta_{<\ell}}^{(\ell-1)}(x)$ denote the complete preparation through
layer $\ell-1$, so that
$U_{\theta_{<\ell}}^{(\ell-1)}(x)\ket0
=\ket{\Phi_x^{(\ell-1)}}$. Define the corresponding reflection by
\begin{equation}
  R_{\Phi}^{(\ell)}
  =\id-2\proj{\Phi_x^{(\ell-1)}}.
  \label{eq:current-reflection}
\end{equation}
Because this entire preceding preparation, including the fixed trainable
gates in layers $1,\ldots,\ell-1$, is known,
\begin{equation}
  R_{\Phi}^{(\ell)}
  =U_{\theta_{<\ell}}^{(\ell-1)}(x)
   (\id-2\proj0)
   U_{\theta_{<\ell}}^{(\ell-1)}(x)^{\dagger}.
...
To display the same parameter dependence directly in the response law, write
the complete incoming state to layer $\ell$ as
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{qfnn_training_trajectories.png}
...
No new benchmark dataset was created for this study. The numerical experiments
use the publicly available MNIST, Fashion-MNIST, and KMNIST datasets,
restricted to their original labels $0,\ldots,7$. The research data generated
during this study---including run configurations, hyperparameter-optimization
records, selected recipes, per-seed histories and checkpoints, aggregate result
tables, and plotting data---are available in the public repository
\url{https://github.com/KiyoLelou10/QFNN}.
...
\url{https://github.com/KiyoLelou10/QFNN}. The archived materials include run configurations, optimization
records, selected recipes, per-seed histories and checkpoints, aggregate
result tables, and plotting data.

\subsection*{Author contributions}
The author conceived the study, developed the architecture and theoretical
results, implemented and validated the software, conducted and analyzed the
numerical experiments, and wrote and revised the manuscript.

\subsection*{Use of generative AI}
The author used OpenAI ChatGPT and Codex as assistive tools for manuscript
organization and drafting, literature searches, code development, mathematical
cross-checking, and language editing. The author critically reviewed and
verified the final manuscript, references, proofs, software, and numerical
...
arXiv:1711.11240. \url{https://doi.org/10.48550/arXiv.1711.11240}
...
\url{https://doi.org/10.48550/arXiv.1812.01718}
...
\url{https://doi.org/10.48550/arXiv.2404.05901}
...
methods. arXiv:2101.11020. \url{https://doi.org/10.48550/arXiv.2101.11020}
